...
## Pros vs. cons of CIT  
Pros:  
  - Non-parametric [does not assume any specific distribution of data: residual normality] 
  - It can model non-linear relationships  
  - The model created is a decision tree, very easy to interpret [very simple model, which is very easy to interpret]  
  - Can be used with both numerical and categorical response variables  
  
Cons:  
  - Can have higher bias [very prone to overfitting]  
...
First, let's specify:  
  ...
  - which **mode** we want to use [regression or classification] 
...
cit_param <- cit_spec %>%
  ...
  update(conditional_min_criterion = 
           conditional_min_criterion())#setting for the specification of the model what are the ranges of the hyperparameters that we want to fine tune
